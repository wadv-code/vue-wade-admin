    ...
    <PopoverContent side="bottom" :side-offset="5" class="w-100 p-0">
      <div class="flex items-center justify-between p-3">
        <div class="space-y-1 pr-2">
          ...
          <div class="text-xs text-muted-foreground mt-2">{{ $t('page.tips.clickToBusiness') }}</div>
        </div>
      ...
      <div class="max-h-96 overflow-y-auto">
        <div v-for="message in messages" :key="message.id"
          ...
          <!-- 通知红点，设置绝对定位到右上角 -->
          <span v-if="!message.isRead" class="absolute top-2 right-2 -mt-1 -mr-1 w-2 h-2 bg-red-500 rounded-full" />
          ...
          <Button variant="link" class="ml-auto" :class="{ 'text-grey': message.isRead }"
