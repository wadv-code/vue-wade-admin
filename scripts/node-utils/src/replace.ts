    ...
    try {
      ...
      if (!matchPattern(itemPath, excludes)) {
        ...
        if (replaceTargets.length)
          await modifyTargetsFile(itemPath, replaceTargets, root);
        // 目录
        const stat = lstatSync(itemPath);
        if (stat.isDirectory())
          await replaceTargetsRecursively(itemPath, targets, excludes, root);
      }
    } catch {
...
function filterPattern(itemPath: string, targets: ReplaceTargets) {
  return targets.filter((target) =>
    minimatch(itemPath, target.pattern, { matchBase: true }),
  );
}
 ...
 */
async function copyAndBackup(
  itemPath: string,
  root: string,
  targets: ReplaceTargets,
) {
  const rootPath = root ?? rootDir;
  ...
  // 组合正则
  const rootReg = new RegExp(rootPath.replace(/\\/g, '/'), 'g');
  ...
  const writePath = normalize(
    itemPath.replace(/\\/g, '/').replace(rootReg, joinPath),
  );
  // 确保目录存在
...
async function start(option: StartReplaceOptions) {
  ...
  const valid = targets.every(
    (s) =>
      (s.target && s.replace) || ((!s.target || !s.replace) && !!s.multiple),
  );
